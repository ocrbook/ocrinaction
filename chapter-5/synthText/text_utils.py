...
class FontState(object):
    ...
    def __init__(self, data_dir='data'):
        ...
        self.fonts = [os.path.join(data_dir, "fonts", f.strip()) for f in os.listdir(
            os.path.join(data_dir, "fonts")) if f.split(".")[-1] in set(["ttf", "ttc"])]
...
class TextSource(object):
    ...
    def __init__(self, min_nchar, fn):
        ...
        self.p_para_nword = [1.7, 3.0, 36]  # beta: (a,b), max_nword
        ...
        while not np.all(self.is_good(lines, f)) and iter < niter:
            ...
            for i in range(nline):
                ...
                if len(lines[i]) > nchar_max:
                    lines[i] = lines[i].strip()[0:nchar_max]
